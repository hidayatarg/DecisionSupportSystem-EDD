...
# import plotly
# plotly.tools.set_credentials_file(username='hidayatarg', api_key='TahXZNWe66TnwxQitPXa')
#
...
def gantt_chart(df, show_today=True, groupby='project'):
    ...
    for i in range(len(df['start_date2'])):
        ax.barh(0.5 * i + 0.5, df['duration'][i], left=df['start_date2'][i], height=0.3, align='center',
                color=df['col'][i], alpha=1.0, label=df['team'][i])
        if (j != df[groupby][i]):
            change = change + 1
            if (change % 2 == 1):
